...
if (socket.request.headers.origin && process.env.FRAPPE_REALTIME_ALLOW_ORIGIN_MISMATCH !== "1" &&
...
if (!socket.request.headers.cookie && !socket.request.headers.authorization) {
	...
	const auth_base = process.env.FRAPPE_REALTIME_AUTH_BASE_URL || `http://127.0.0.1:${conf.webserver_port || 8000}`;
	let auth_req = request.get(`${auth_base}/api/method/frappe.realtime.get_user_info`);
